...
const columns = [
...

    {
        ...
        Cell: (row) => {
          ...
          const imageUrl = imageName ? `http://localhost:4001/productattribute_svg/${imageName}` : null;
        ...
        
    {
      ...
      Cell: (row) => {
        ...
        const handleDelete = async () => {
          try {
            await axios.get(`http://localhost:4001/api/v1/pattern/delete/${row.row.original.id}`);
        ...
        return (
          <div className="flex space-x-3 rtl:space-x-reverse">
            ...
            <Tooltip content="Edit" placement="top" arrow animation="shift-away">
               <Link href={`/pattern/edit/${row.row.original.id}`} className="action-btn">
...
function page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:4001/api/v1/pattern');
  ...
  return (
    <>
            ...
            <div className="col-span-2">
                <div className="col-span-2">
                    <CustomTable  columns={columns} data={data} title="Patterns" />
                </div>
            </div>
