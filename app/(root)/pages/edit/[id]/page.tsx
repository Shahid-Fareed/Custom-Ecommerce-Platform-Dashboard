...
function page({ params }: any) {
  ...
  useEffect(() => {
    axios.get(`http://localhost:4001/api/v1/pages/details/${id}`)
      .then(response => {
  ...
  const handleSubmit = async (event: any) => {
    ...
    formData.append("meta_title", page.meta_title);
    formData.append("meta_description", page.meta_description);
    formData.append("status", page.status);
    try {
      const response = await axios.post(
        `http://localhost:4001/api/v1/pages/update/${id}`,
        formData,
        {
          headers: {
