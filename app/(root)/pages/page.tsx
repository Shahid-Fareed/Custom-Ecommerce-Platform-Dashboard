  ...
  },
  {
    ...
    Cell: (row) => {
      ...
      const imageUrl = imageName
        ? `http://localhost:4001/pages_image/${imageName}`
        : null;
...

  {
    ...
    Cell: (row) => {
      const handleDelete = async () => {
        try {
          await axios.get(
            `http://localhost:4001/api/v1/pages/delete/${row.row.original.id}`
          );
          // Update the table data by removing the deleted item
          setData((prevData) =>
            prevData.filter((item) => item.id !== row.row.original.id)
          );
        } catch (error) {
      ...
      return (
        <div className="flex space-x-3 rtl:space-x-reverse">
          ...
          <Tooltip content="Edit" placement="top" arrow animation="shift-away">
            <Link
              href={`/pages/edit/${row.row.original.id}`}
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:4001/api/v1/pages/");
  ...
  return (
    <>
      ...
      <div className="col-span-2">
        <div className="col-span-2">
          <CustomTable columns={columns} data={data} title="Pages" />
        </div>
      </div>
    </>
  );
