...
const columns = [
    ...
    },
    {
      ...
      Cell: (row) => {
        ...
        const imageUrl = imageName ? `http://localhost:4001/sections_image/${imageName}` : null;
        ...
        
    {
      ...
      Cell: (row) => {
        ...
        const handleDelete = async ({params}:any) => {
          ...
          try {
            await axios.get(`http://localhost:4001/api/v1/sections/delete/${row.row.original.id}`);
        ...
        return (
          <div className="flex space-x-3 rtl:space-x-reverse">
            ...
            <Tooltip content="Edit" placement="top" arrow animation="shift-away">
               <Link href={`/pages/${p_id}/sections/edit/${row.row.original.id}`} className="action-btn">
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:4001/api/v1/sections/${p_id}`);
  ...
  return (
    <>
            ...
            <div className="col-span-2">
                <div className="col-span-2">
                    <CustomTable  columns={columns} data={data} title="Sections" />
                </div>
            </div>
