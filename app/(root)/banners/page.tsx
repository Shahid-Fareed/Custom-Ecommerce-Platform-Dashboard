...
const columns = [
...

    {
        ...
        Cell: (row) => {
          ...
          const imageUrl = imageName ? `http://localhost:4001/banner_image/${imageName}` : null;
...

    {
        ...
        accessor: "url",
        Cell: (row) => {
          return <a href ="/#"><span>{row?.cell?.value}</span></a>;
        },
      },

    {
        ...
        accessor: "is_primary",
        Cell: (row) => {
          const primaryValue = row?.cell?.value;
    
          return (
            <span className="block w-full">
              <span
                className={`inline-block px-3 min-w-[90px] text-center mx-auto py-1 rounded-[999px] bg-opacity-25 ${
                    primaryValue === 1 ? "text-success-500 bg-success-500" : ""
                } 
                ${primaryValue === 0 ? "text-danger-500 bg-danger-500" : ""}
                `}
              >
                {primaryValue === 1 ? "Yes" : primaryValue === 0 ? "No" : ""}
              </span>
            </span>
          );
        },
        ...
        
    {
      ...
      Cell: (row) => {
        ...
        const handleDelete = async () => {
          try {
            await axios.get(`http://localhost:4001/api/v1/banner/delete/${row.row.original.id}`);
        ...
        return (
          <div className="flex space-x-3 rtl:space-x-reverse">
            ...
            <Tooltip content="Edit" placement="top" arrow animation="shift-away">
               <Link href={`/banners/edit/${row.row.original.id}`} className="action-btn">
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:4001/api/v1/banner');
  ...
  return (
    <>
            ...
            <div className="col-span-2">
                <div className="col-span-2">
                    <CustomTable  columns={columns} data={data} title="Banners" />
                </div>
            </div>
