...
const columns = [
...

    {
        ...
        Cell: (row) => {
          ...
          const imageUrl = imageName ? `http://localhost:4001/photowall_image/${imageName}` : null;
        ...
        
    {
      ...
      Cell: (row) => {
        ...
        const handleDelete = async () => {
          try {
            await axios.get(`http://localhost:4001/api/v1/photowall/delete/${row.row.original.id}`);
        ...
        return (
          <div className="flex space-x-3 rtl:space-x-reverse">
            ...
            <Tooltip content="Edit" placement="top" arrow animation="shift-away">
               <Link href={`/photowall/edit/${row.row.original.id}`} className="action-btn">
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:4001/api/v1/photowall');
  ...
  return (
    <>
            ...
            <div className="col-span-2">
                <div className="col-span-2">
                    <CustomTable  columns={columns} data={data} title="Photowalls" />
                </div>
            </div>
