...
function page({ params }: any) {
  ...
  useEffect(() => {
    axios.get(`http://localhost:4001/api/v1/color/details/${id}`)
      .then(response => {
  ...
  const handleSubmit = (event: any) => {
    ...
    axios.post(`http://localhost:4001/api/v1/color/update/${id}`, color)
      .then(response => {
            ...
            <div className="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 gap-5 pt-10">
              ...
              <Select
                ...
                selectedValues={{
                  value: color.status,
                  label:
                    options.find((option) => option.value === color.status)
                      ?.label || "",
                }}
              />
